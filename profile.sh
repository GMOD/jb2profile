...
for i in 28x 56x 112x 224x 448x 896x 1792x; do
  for j in bam cram; do
    echo $i $j
    profile "ctgA:19,000-20,000" "$i.shortread.$j"  "results/$i-1kb-shortread-$j.json" "volvox" $i_$j_shortread_1kb.txt
    profile "ctgA:19,000-29,000" "$i.shortread.$j"  "results/$i-10kb-shortread-$j.json" "volvox" $i_$j_shortread_10kb.txt
    profile "ctgA:15,000-34,000" "$i.shortread.$j"  "results/$i-19kb-shortread-$j.json" "volvox" $i_$j_shortread_19kb.txt
...
for i in 40x 80x 160x 320x 640x; do
  for j in bam cram; do
    echo $i $j
    profile "ctgA:19,000-20,000" "$i.longread.$j"  "results/$i-1kb-longread-$j.json" "volvox" $i_$j_longread_19kb.txt
    profile "ctgA:19,000-29,000" "$i.longread.$j"  "results/$i-10kb-longread-$j.json" "volvox" $i_$j_longread_19kb.txt
    profile "ctgA:15,000-34,000" "$i.longread.$j"  "results/$i-19kb-longread-$j.json" "volvox" $i_$j_longread_19kb.txt
  done;
done;
